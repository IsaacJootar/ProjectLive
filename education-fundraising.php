...
if(!isset($_SESSION['SESS_USER'])){
  include ('includes/header.php');
   ...
   include ('includes/header-login.php');  
    ...
    ?>
<body onLoad="window.scroll(0, 600)"> <?php  // this snippet scroll the page to a certian page. enuf to view the campaigns as they display?>
<style type="text/css">
body,td,th {
  ...
  <div class="main-content">
    ...
    <section class="inner-header divider layer-overlay overlay-dark" data-bg-img="images/bg/bg2.jpg">
        ...
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-12  text-center">
              <h2 class="text-white font-34" align="center">PROJECTLIVE OFFERS 12 CATEGORIES FOR AN EASY FUNDRAISING EXPERIENCE. MAKE A DREAM COME TRUE. RAISE MONEY FOR YOURSELF AND OTHERS. </h2>
      ...
      <div class="container" align="center">
        ...
        <p>&nbsp;</p>
        <p><a href="campaigns/start" class="btn btn-dark btn-theme-colored btn-x3">START AN EDUCATION CAMPAIGN </a></p>
Raise Money for Everything Education<p>
Education is a vital entry point into the society, and Projectlive is here to help you obtain get that dream degree. From fees, to books, technology and more, Projectlive crowdfunding campaign can help you get supplies you need for your classroom activities. And because attending the University is very expensive, Projectlive is committed to helping you ease the financial burden higher education costs. If you would like to study abroad but constrained by a lack of financial resources, just start a fundraising campaign on Projectlive.<p>
Projectlive provides a great way to raise money for tuition, classroom technology or supplies, and any kind of educational need.         </div>
    <section>
      <div class="container">
        <div class="row">
          <div class="col-md-12">
           ...
           <?php  //determine active tabs//
               $category='Education'; // project category page default
            ...
            if(isset($_GET['ref'])){
              ...
              switch ($_GET['ref']) {
                  ...
                  $now=time(); 
                  $get_campaigns=campaignsManager:: find_by_sql("SELECT `pl_campaign_basics`. `campaign_id`,  `pl_campaigns`. `id` FROM `pl_campaign_basics` JOIN `pl_campaigns` ON `pl_campaign_basics`. `campaign_id`=`pl_campaigns`. `id`  WHERE `pl_campaigns`.`campaign_category` = '{$category}' AND `pl_campaign_basics`. `campaign_due_date` - $now <= 345600  AND `pl_campaign_basics`. `campaign_state` = 'live'");
                  break;
                  case 'ended':
                  $active_tab=5;
                  $get_campaigns=campaignsManager:: find_by_sql("SELECT `pl_campaign_basics`. `campaign_id`,  `pl_campaigns`. `id` FROM `pl_campaign_basics` JOIN `pl_campaigns` ON `pl_campaign_basics`. `campaign_id`=`pl_campaigns`. `id`  WHERE `pl_campaigns`.`campaign_category` = '{$category}' AND `pl_campaign_basics`. `campaign_state` = 'ended'"); 
                  break;
                  default: // default should reference the all category tab//
                  $active_tab=1;
                  $get_campaigns=campaignsManager:: find_by_sql("SELECT `pl_campaign_basics`. `campaign_id`,  `pl_campaigns`. `id` FROM `pl_campaign_basics` JOIN `pl_campaigns` ON `pl_campaign_basics`. `campaign_id`=`pl_campaigns`. `id`  WHERE `pl_campaigns`.`campaign_category` = '{$category}' AND `pl_campaign_basics`. `campaign_state` != 'declined' AND `pl_campaign_basics`. `campaign_state` != 'build up' AND `pl_campaign_basics`. `campaign_state` != 'review' ");
                  break;
              }
            ...
            <div class="horizontal-tab-centered text-left ">
                <ul class="nav nav-pills mb-8">
                 <li class="<?php if($active_tab==1){echo 'active';} else {echo '';}?>"> <a href="education-fundraising?ref=all" data-toggle="tooltip" data-placement="right" title="This will display projects under the education category but with no particular order." class="active">All</a> </li>

                  <li class="<?php if($active_tab==2){echo 'active';} else {echo '';}?>"> <a href="education-fundraising?ref=recent" data-toggle="tooltip" data-placement="right" title="This will display the current projects under education category."  class=""><strong>Recent Projects</strong></a></li>
                   <li class="<?php if($active_tab==3){echo 'active';} else {echo '';}?>"><a href="education-fundraising?ref=most funded" data-toggle="tooltip" data-placement="right" title="This will display projects under the education category that are most funded."  class=""><strong>Most Funded</strong></a> </li>
                 
                  <li class="<?php if($active_tab==4){echo 'active';} else {echo '';}?>"> <a href="education-fundraising?ref=ending soon" data-toggle="tooltip" data-placement="right" title="This will display projects under the education category that are ending soon."  class=""><strong>Ending Soon</strong></a> </li>
                   <li class="<?php if($active_tab==5){echo 'active';} else {echo '';}?>"> <a href="education-fundraising?ref=ended" data-toggle="tooltip" data-placement="right" title="This will display projects under the education category already ended." class=""><strong>Ended</strong></a>
              </li>
                </ul>
            </div>
...
<?php // if reference is not set, display the filter guide that carries ALL, just as the using in the switch case above. this issue is neccessary bcus on initail page visit $_GET is not yet set atall//
      ...
      if(!isset($_GET['ref'])){
        ...
        echo '<i style="float:left">Showing campaigns under <strong class="text-theme-colored">Education</strong> that are: <strong class="text-theme-colored">Available</strong></i>';
      }?>

<?php  // if reference is set, and the  filter category selected is all then change the grammer//
     if(isset($_GET['ref'])){
        echo '<i style="float:left">Showing campaigns under <strong class="text-theme-colored">Education</strong> that are: <strong class="text-theme-colored">';
