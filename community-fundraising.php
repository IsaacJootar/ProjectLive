  ...
  <div class="main-content">
    ...
    <section class="inner-header divider layer-overlay overlay-dark" data-bg-img="images/bg/bg2.jpg">
      ...
      <div class="container" align="center">
        ...
        <p>&nbsp;</p>
        <p><a href="campaigns/start"  class="btn btn-dark btn-theme-colored btn-x3">START A COMMUNITY CAMPAIGN </a></p>
Raise money to make your community a better place. <p>
If you are passionate about doing something to improve the state of your community, don’t despair at lack of funds; raise money on Projectlive for that ambition today. Is it water supply, environment sanitation measures, or perhaps security? Projectlive will help you improve your community whatever your cause is.        </div>
    <section>
      <div class="container">
        <div class="row">
          <div class="col-md-12">
           ...
           <?php  //determine active tabs//
               $category='Community'; // project category page default
            ...
            if(isset($_GET['ref'])){
              ...
              switch ($_GET['ref']) {
                  ...
                  case 'most funded':
                  $active_tab=3;
                  $get_campaigns=campaignsManager:: find_by_sql("SELECT SUM(`pl_campaign_donations`.`donation_amount`) AS `donation_amount`,  `pl_campaign_donations`. `campaign_id`, `pl_campaign_donations`. `donation_amount`, `pl_campaigns`. `id` FROM `pl_campaign_donations` JOIN `pl_campaigns` ON `pl_campaign_donations`. `campaign_id`=`pl_campaigns`. `id` WHERE `pl_campaigns`.`campaign_category` = '{$category}' GROUP BY `pl_campaigns`. `id` ORDER BY `pl_campaign_donations`. `campaign_id` DESC"); 
                  break;  
            ...
            <div class="horizontal-tab-centered text-left ">
                <ul class="nav nav-pills mb-8">
                 <li class="<?php if($active_tab==1){echo 'active';} else {echo '';}?>"> <a href="community-fundraising?ref=all" data-toggle="tooltip" data-placement="right" title="This will display projects under the community category but with no particular order." class="active">All</a> </li>

                  <li class="<?php if($active_tab==2){echo 'active';} else {echo '';}?>"> <a href="community-fundraising?ref=recent" data-toggle="tooltip" data-placement="right" title="This will display the current projects under community category."  class=""><strong>Recent Projects</strong></a></li>
                   <li class="<?php if($active_tab==3){echo 'active';} else {echo '';}?>"><a href="community-fundraising?ref=most funded" data-toggle="tooltip" data-placement="right" title="This will display projects under the community category that are most funded."  class=""><strong>Most Funded</strong></a> </li>
                 
                  <li class="<?php if($active_tab==4){echo 'active';} else {echo '';}?>"> <a href="community-fundraising?ref=ending soon" data-toggle="tooltip" data-placement="right" title="This will display projects under the community category that are ending soon."  class=""><strong>Ending Soon</strong></a> </li>
                   <li class="<?php if($active_tab==5){echo 'active';} else {echo '';}?>"> <a href="community-fundraising?ref=ended" data-toggle="tooltip" data-placement="right" title="This will display projects under the community category already ended." class=""><strong>Ended</strong></a>
              </li>
                </ul>
            </div>
...
<?php // if reference is not set, display the filter guide that carries ALL, just as the using in the switch case above. this issue is neccessary bcus on initail page visit $_GET is not yet set atall//
      ...
      if(!isset($_GET['ref'])){
        ...
        echo '<i style="float:left">Showing campaigns under <strong class="text-theme-colored">community</strong> that are: <strong class="text-theme-colored">Available</strong></i>';
      }?>

<?php  // if reference is set, and the  filter category selected is all then change the grammer//
     if(isset($_GET['ref'])){
        echo '<i style="float:left">Showing campaigns under <strong class="text-theme-colored">community</strong> that are: <strong class="text-theme-colored">';
